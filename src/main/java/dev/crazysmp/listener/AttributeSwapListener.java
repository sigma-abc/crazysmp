...
public class AttributeSwapListener implements Listener {
    ...
    private void refreshAttributes(Player player) {
        // Paper 1.21.1 uses GENERIC_ prefixed attribute names.
        // (Short names like Attribute.MAX_HEALTH only exist in Paper 1.21.5+)
        tickle(player, Attribute.GENERIC_MAX_HEALTH);
        tickle(player, Attribute.GENERIC_ATTACK_DAMAGE);
        tickle(player, Attribute.GENERIC_ATTACK_SPEED);
        forceTickle(player, Attribute.GENERIC_ARMOR);
        forceTickle(player, Attribute.GENERIC_ARMOR_TOUGHNESS);
        forceTickle(player, Attribute.GENERIC_KNOCKBACK_RESISTANCE);
        forceTickle(player, Attribute.GENERIC_MOVEMENT_SPEED);
    }
